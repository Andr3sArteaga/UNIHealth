...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        # Try to create patient profile if table exists and user is a patient
        if role_name == "user":
            try:
                # Map the registration data to the available fields
                ...
                PerfilPaciente.objects.create(
                    ...
                    contacto_emergencia=serializer.validated_data.get("contacto_emergencia_nombre"),
                    alergias=serializer.validated_data.get("alergias", ""),
                    antecedentes=f"Medicamentos: {serializer.validated_data.get('medicamentos_actuales', '')}\nCondiciones: {serializer.validated_data.get('condiciones_medicas', '')}\nTipo sangre: {serializer.validated_data.get('tipo_sangre', '')}"
                )
            except Exception as e:
                # If profile creation fails, continue without it
                print(f"Warning: Could not create patient profile: {e}")
